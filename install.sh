...
ExecStart=$DAEMON_DIR/.venv/bin/python $DAEMON_DIR/whisper_daemon.py --model $WHISPER_CPP_DIR/models/ggml-$MODEL_NAME.bin --whisper-cli $WHISPER_CPP_DIR/build/bin/whisper-cli --no-notifications
Restart=on-failure
RestartSec=5
